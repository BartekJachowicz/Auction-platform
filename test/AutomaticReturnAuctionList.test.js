...
const AutomaticReturnAuctionList = artifacts.require('./AutomaticReturnAuctionList.sol')

contract('AutomaticReturnAuctionList', (accounts) => {
    before(async () => {
        this.auctionList = await AutomaticReturnAuctionList.deployed()
    })
    ...
    it('get sum of previous bids', async () => {
        date = new Date(2021, 06, 12)
        var deadline = date.getTime()/1000
        var result = await this.auctionList.createAuction('Auction3', 1, deadline)

        const SMALLEST_TICK_IN_WEI = 500000000000000;
        var result1 = await this.auctionList.makeBid(3, 2 + SMALLEST_TICK_IN_WEI, {value: 2 + SMALLEST_TICK_IN_WEI, from: accounts[0]})
        var result2 = await this.auctionList.makeBid(3, 4 + 2 * SMALLEST_TICK_IN_WEI, {value: 4 + 2 * SMALLEST_TICK_IN_WEI, from: accounts[0]})
        var result3 = await this.auctionList.getSumOfPreviousBids(3, {from: accounts[0]})
        var event = result3
        assert.equal(event[0].toNumber(), 3)
        assert.equal(event[1].toNumber(), 6 + 3 * SMALLEST_TICK_IN_WEI)
    })
})